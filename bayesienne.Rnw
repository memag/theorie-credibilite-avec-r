...
\chapter{Crédibilité bayésienne}
...
etc. On remarquera qu'aucune hypothèse à priori n'est faite sur
$\theta$, on «laisse parler les données».

Dans l'approche bayésienne, l'opinion à priori d'un individu sur la
...
aléatoire (l'information) s'accumulent, l'opinion à priori sur la valeur
possible de $\theta$ est révisée et améliorée. On calcule alors
$u(\theta|x_1, \dots, x_n)$, la distribution à postériori de $\Theta$,
...
paramètre $\theta$ est obtenu en minimisant l'espérance à postériori
...
Dans un tel cas l'estimateur bayésien minimisant
\begin{displaymath}
...
\begin{quote}
  Dans l'\autoref{ex:bayesienne:introduction}, la variable
...
\fancybreak{\pfbreakdisplay}
\begin{quote}
  Dans l'\autoref{ex:bayesienne:introduction}, on a
  \begin{align*}
    U(\theta)
    &=
    \begin{cases}
...
fonction $g^*(S_{i1}, \dots, S_{in})$ est la \emph{prime bayésienne}
\begin{align*}
  B_{i, n + 1}
...
\end{align*}
où $u(\theta|x_1, \dots, x_n)$ est la distribution à postériori des
...
\begin{rems}
  \begin{enumerate}
  \item La prime bayésienne est la meilleure prévision de $S_{i, n +
      1}$ que l'on puisse calculer.
  \item Comme la prime collective, la prime bayésienne est une moyenne
    pondérée des primes de risque, mais utilisant la distribution à
    postériori de $\Theta$ plutôt que la distribution à priori;
    comparer
    \begin{align*}
  ...
  \item À l'inverse, on peut interpréter la prime collective comme la
    prime bayésienne de première année, lorsque aucune expérience
    n'est disponible.
...
\begin{exemple}
  ...
  \item Calculer la prime bayésienne pour la troisième année si
    $S_1 = 2$ et $S_2 = 1$.

    \begin{enumerate}[1.]
    \item Calculer la distribution à postériori de $\Theta$:
      \begin{displaymath}
      ...
      \begin{displaymath}
        ...
        \end{cases}
      \end{displaymath}
    \item Calculer la prime bayésienne:
      \begin{align*}
        B_3
...
\begin{exemple}
  ...
  Considérer le portefeuille simplifié de
  l'\autoref{ex:introduction-historique:simplifie}. Un contrat ne peut
  avoir au maximum qu'un seul sinistre de montant 1 par année. (En
  d'autres termes, l'expérience consiste en une suite de 1 et de 0
  selon qu'il y a eu un sinistre dans une année ou non.) La
  probabilité d'avoir un sinistre est toutefois inconnue et
  potentiellement différente pour chaque contrat.
  ...
  \item Calculer la prime collective.
    ...
    \end{align*}
  \item Calculer la prime bayésienne après $n$ années.

    Tout d'abord, la distribution à postériori de $\Theta$ après $n$
...
\begin{exemple}
  ...
  On reprend le modèle de l'\autoref{ex:bayesienne:bernoulli-beta},
  soit
  \begin{displaymath}
    S_t|\Theta = \theta \sim \text{Bernoulli}(\theta),
  \end{displaymath}
...
On a déjà vu à la \autoref{sec:bayesienne:prevision:collective} que
\begin{displaymath}
...
\begin{rem}
  ...
  à priori de $\Theta$ pour la première et de la distribution à
  postériori pour la seconde. Par conséquent,
  \begin{center}
...
Du \autoref{thm:bayesienne:predictive}, on a
\begin{align*}
...
\section{Crédibilité bayésienne linéaire (ou exacte)}
\label{sec:bayesienne:lineaire}

La prime bayésienne de l'\autoref{ex:bayesienne:bernoulli-beta}
...
que la prime bayésienne est une prime de crédibilité pour certaines
...
\begin{exemple}[Cas Poisson/gamma]
  ...
  \item Calculer la prime bayésienne à partir de la distribution à
    postériori de $\Theta$.
    ...
    conséquent, la prime bayésienne est
    \begin{align*}
      B_{n + 1}
      &= \esp{\mu(\Theta)|S_1, \dots, S_n} \\
      &= \esp{\Theta|S_1, \dots, S_n} \\
  ...
  \item Calculer la prime bayésienne à partir de la distribution
    prédictive.
  ...
  \item Calculer $\var{S}$.
    ...
    \item Directement depuis la marginale:
      ...
      \end{align*}
    \end{enumerate}
  \item Calculer la prime bayésienne pour les dix prochaines années si
    à priori $\Theta \sim \text{Gamma}(3, 3)$ et que les montants de
    sinistres au cours de ces années sont les suivants: 5, 3, 0, 1, 1,
    2, 0, 2, 0, 2.

    La prime bayésienne dans le cas Poisson/gamma est
    \begin{displaymath}
    ...
    Conclusions:
    \begin{itemize}
    \item La distribution à postériori de $\Theta$ est de plus en plus
    ...
    \item La précision de la prime bayésienne s'améliore (la vraie
...
\begin{exemple}[Cas normale/normale]
  ...
  que la distribution à priori, on peut immédiatement conclure que
  \begin{displaymath}
  ...
  de la distribution à postériori ou de la prédictive,
  \begin{align*}
    B_{n+1}
    &= \tilde{\mu} \\
...
prime bayésienne linéaire (plus leurs convolutions):
\begin{enumerate}
\item Poisson/gamma
...
\item De plus, $z = n/(n + K) \rightarrow 1$ lorsque $K \rightarrow 0$
  ...
  \autoref{fig:bayesienne:gamma}). On accorde donc peu de poids à la prime
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    $\esp{S_2|S_1 = 4}$. On obtient la distribution à postériori de
    $\Theta|S_1 = 4$ avec
    \begin{displaymath}
      \Pr[\Theta = \theta|S_1 = 4]
...
\begin{exercice}
  ...
  \item Une petite expérience pratique maintenant, aussi ludique
    ...
    et 1 pour face. Après chaque lancer, calculer la distribution à
    postériori de $\Theta$ et en faire un graphique approximatif. On
...
@
  ...
  \begin{sol}
    ...
    \item Une pièce a été lancée 10 fois et les résultats suivants
      ...
      présentées aux figures \ref{fig:bayesienne:beta0}--\ref{fig:bayesienne:beta5}. Une
  ...
  \begin{sol}
    ...
    \item Par la loi des probabilités totales,
      \begin{align*}
        ...
        &= 0,2257.
      \end{align*}
    \item On trouve d'abord la densité à postériori de $\Theta$:
      \begin{align*}
        u(\theta|x_1=1, x_2=1)
...
\begin{exercice}
  ...
  100\theta]$.
  \begin{enumerate}
  \item Trouver la prime bayésienne si des résultats de 80 et 340 ont
...
\begin{exercice}
  ...
  Quelle est maintenant la distribution à postériori de $\Theta$ pour
  ce simulateur?
  \begin{rep}
  ...
  \begin{sol}
    ...
    \item Le modèle est le suivant: $N|\Theta \sim
      ...
      \begin{displaymath}
        ...
        \end{cases}
      \end{displaymath}
    \item On doit d'abord trouver la distribution à postériori de
      $\Theta$ en utilisant
      \begin{align*}
...
\begin{exercice}
  ...
  cours des deux premières années. Calculer la prime bayésienne de ce
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item Comme la densité à postériori de $\Theta$ est du même type
      que sa densité à priori, on peut calculer en premier la densité
    ...
    \item On a
      \begin{align*}
        ...
        &= \theta^{\alpha + \sum x_t - 1} (1 - \theta)^{\beta + n \nu
          + \sum x_t - 1}.
      \end{align*}
      La distribution à postériori de $\Theta$ sachant l'expérience
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item De la distribution marginale de $S$, on a $m = \esp{S} =
      \tau \lambda/(\alpha - 1)$ (\autoref{distributions:paretogen}).
...
\begin{exercice}
  ...
  $\Theta \sim \mathrm{Gamma}(7, 42)$, la prime bayésienne de
...
\begin{exercice}
  ...
  Exponentielle$(\theta)$ et que la distribution à priori de $\Theta$
  ...
  \end{rep}
  \begin{sol}
    On a un modèle exponentielle/gamma. Selon le tableau de
...
\begin{exercice}
  ...
  \cite{Jewell:exact:1974}, à savoir que la prime bayésienne issue d'un
  ...
  \begin{sol}
    \begin{enumerate}
    \item On doit démontrer que la densité à postériori $u(\theta|x_1,
      \dots, x_n)$ est de la même famille que la densité à priori
    ...
    \item On sait que la prime bayésienne est de la même forme que la
...
\begin{exercice}
  ...
  de $\Theta$.
  \begin{sol}
    Voir l'\autoref{ex:bayesienne:gamma_gamma}.
  \end{sol}
\end{exercice}
