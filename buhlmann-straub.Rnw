...
\chapter{Le modèle de Bühlmann--Straub}
...
\begin{proof}
  ...
  Les deux premiers résultats sont équivalents à ceux du théorème
  \ref{thm:buhlmann:covariances} à la seule différence que
  \begin{align*}
...
\begin{thm}
  ...
  homogène de la prime de risque $\mu(\Theta_i)$ --- ou de $X_{i, n +
    1}$ --- est
  \begin{displaymath}
    \pi_{i, n + 1}^{\mathrm{BS}} = z_i X_{iw} + (1 - z_i) m
  \end{displaymath}
  où
  \begin{align*}
    z_i
...
\begin{proof}
  La démonstration est similaire à celle du théorème
  \ref{thm:buhlmann:prime}. Comme précédemment, on peut se restreindre
...
Du chapitre \ref{chap:buhlmann}, on soupçonne que l'estimateur intuitif
\begin{displaymath}
...
\item Calculer les primes de crédibilité
  \begin{displaymath}
    \hat{\pi}_{i, n + 1}^{\mathrm{BS}} = \hat{z}_i X_{iw}
...
montants de sinistres moyens $X_{it}$ sont présentés au tableau
\ref{tab:ratios} (à noter que le tableau est transposé par rapport à
la notation usuelle).
...
Au tableau \ref{tab:poids}, on trouvera les poids $w_{it}$ associés
...
\begin{align*}
  ...
  \hat{a}   &= \nombre{72310},
\end{align*}
ce qui mène aux résultats du tableau \ref{tab:buhl}. Le facteur de
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \nombre{2064} & \nombre{1511} & \nombre{1822} & \nombre{1360} &
    \nombre{1599} \\
    Prime de crédibilité $\pi_{i, 13}^{\mathrm{B}}$ &
    \nombre{2044} & \nombre{1519} & \nombre{1814} & \nombre{1376} &
    \nombre{1602} \\
...
Le tableau \ref{tab:poids} montre que le poids relatif de chacun des
...
les primes de crédibilité du tableau \ref{tab:bs} ont été calculés
...
\begin{table}[tbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \nombre{2061} & \nombre{1511} & \nombre{1806} & \nombre{1353} &
    \nombre{1600} \\
    Prime de crédibilité $\pi_{i, 13}^{\mathrm{BS}}$ &
    \nombre{2053} & \nombre{1529} & \nombre{1790} & \nombre{1468} &
    \nombre{1605} \\
    Facteur de crédibilité $z$ &
...
tableaux \ref{tab:buhl} et \ref{tab:bs} sont les plus marquées. La
prime de crédibilité de cet état augmente en effet de \nombre{1376} à
...
\begin{exercice}
  ...
  \begin{sol}
    On calcule d'abord l'espérance de la variable aléatoire $(X_{it} -
    X_{iw})^2$. En utilisant les résultats du problème
    \ref{prob:buhlmann-straub:covariances}, on a que
    \begin{align*}
      \esp{(X_{it} - X_{iw})^2}
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \ref{chap:buhlmann}.\ref{ex:buhlmann:simulation}. Il faut
