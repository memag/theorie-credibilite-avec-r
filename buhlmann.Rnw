...
\begin{enumerate}
\item la prime bayesienne est une prime de crédibilité dans certains
  cas seulement;
...
\begin{corrolaire}
  En posant $X \equiv Y$ dans le théorème
  \ref{thm:buhlmann:covariance_iteree}, on obtient
  \begin{displaymath}
...
en crédibilité bayesienne, mais on relâche légèrement les hypothèses.
...
approximation linéaire de la prime bayesienne.
...
\begin{corrolaire}
  ...
  \end{displaymath}
  l'égalité survenant lorsque la prime bayesienne est une prime de
  crédibilité.
\end{corrolaire}
...
\item trouver la meilleure approximation linéaire de la prime
  bayesienne (prime de crédibilité).
\end{enumerate}
...
\begin{exemple}[Bernoulli/uniforme]
  On a vu à l'exemple \ref{ex:bayesienne:bernoulli-uniforme} que si
  \begin{align*}
    S_t|\Theta = \theta &\sim \text{Bernoulli}(\theta) \\
    \Theta &\sim U(a, b),
  \end{align*}
  alors la prime bayesienne est très compliquée. Ici, $\mu(\theta)
...
\begin{exemple}
  ...
  \begin{align*}
    ...
    &= \frac{n}{n + \alpha - 1},
  \end{align*}
  tel qu'obtenu à l'exemple
  \ref{ex:bayesienne:poisson-exponentielle}. %
  \qed
\end{exemple}
...
Avec l'approche non paramétrique, nous délaissons l'approche bayesienne
pure pour l'approche bayesienne \emph{empirique}.
...
Les figure \ref{fig:buhlmann:s2} et \ref{fig:buhlmann:a} illustrent
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \begin{enumerate}
    \item De manière similaire à l'exercice
      \ref{chap:bayesienne}.\ref{ex:bayesienne:YARD}, on trouve la
...
\begin{exercice}
  ...
  \item Sachant que $N_1 = 0$, estimer $N_2$ par l'approche bayesienne
    pure.
  ...
  \begin{sol}
    ...
    de l'annexe \ref{chap:formules}. La prime bayesienne et la prime
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    pertinentes à l'annexe \ref{chap:formules}.
    \begin{enumerate}
    \item On a $m = \alpha/\lambda
      = 0,75$.
    \item La prime de crédibilité est égale à la prime bayesienne,
      soit
      \begin{align*}
        \pi_{n + 1}
