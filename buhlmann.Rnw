...
\begin{rems}
  ...
  \item La prime de crédibilité a deux belles propriétés:
    ...
    \item puisque
      ...
      \mu(\Theta_i)$.
    \end{enumerate}
  \item Puisque $\pi_{i, n + 1}^{\text{B}}$ est sans biais, une
...
\item $U(\theta)$ est la \emph{fonction de structure} du portefeuille:
  \begin{itemize}
  \item avant: opinion à priori de l'assureur sur le niveau de risque
    d'un contrat;
...
\begin{rems}
  ...
  \item Si $\bar{S}_1 = \dots = \bar{S}_I = \bar{S}$, alors $a = 0$ et
    la prime de crédibilité est $\pi_{i, n + 1}^{\text{B}} = \bar{S} =
